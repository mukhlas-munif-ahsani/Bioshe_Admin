package com.munifahsan.biosheadmin.ui.tambahProdukPilihFoto
...
import com.munifahsan.biosheadmin.databinding.ActivityPilihFotoBinding
...
class PilihFotoActivity : AppCompatActivity() {
    private lateinit var binding: ActivityPilihFotoBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityPilihFotoBinding.inflate(layoutInflater)
    ...
    private fun postImage(imageUri: Uri){
        ...
        Constants.PRODUCT_DB.document(produkId).collection("IMAGES").whereEqualTo("nomor", 0).get().addOnSuccessListener {
            if (it.isEmpty){
                val data = mapOf(
                    "image" to imageUri.toString(),
                    "nomor" to 0
                )
                Constants.PRODUCT_DB.document(produkId).collection("IMAGES").document().set(data).addOnCompleteListener {
                    finish()
                }
                Constants.PRODUCT_DB.document(produkId).update("thumbnail", imageUri.toString())
            } else {
                val data = mapOf(
                    "image" to imageUri.toString(),
                    "nomor" to 1
    ...
    private fun customCompressImage() {
        actualImage?.let { imageFile ->
            lifecycleScope.launch {
                ...
                // Full custom
                compressedImage = Compressor.compress(this@PilihFotoActivity, imageFile) {
                    resolution(1280, 720)
                    quality(80)
