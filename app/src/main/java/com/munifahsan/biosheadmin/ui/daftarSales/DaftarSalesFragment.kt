package com.munifahsan.biosheadmin.ui.daftarSales
...
import com.munifahsan.biosheadmin.databinding.FragmentDaftarSalesBinding
...
class DaftarSalesFragment : Fragment() {

    private var _binding: FragmentDaftarSalesBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentDaftarSalesBinding.inflate(layoutInflater, container, false)
    ...
    private fun showDaftar() {
        binding.rvSales.layoutManager = LinearLayoutManager(
            activity,
            LinearLayoutManager.VERTICAL,
            false
        )
        val rootRef = FirebaseFirestore.getInstance()
        val query = rootRef.collection("SALES").orderBy("nama", Query.Direction.ASCENDING)

        val options = FirestoreRecyclerOptions
            .Builder<Sales>()
            .setQuery(query, Sales::class.java)
            .build()
        ...
        fun setContentDistributor(
            ...
            view.findViewById<RelativeLayout>(R.id.relShowall).setOnClickListener {
                if (CheckConection.isNetworkAvailable(activity!!)) {
                    val intent = Intent(activity, DetailSalesActivity::class.java)
    ...
    private inner class DaftarFirestoreRecyclerAdapter(options: FirestoreRecyclerOptions<Sales>) :
        FirestoreRecyclerAdapter<Sales, ProductViewHolder>(
            options
        ) {
