...
class DaftarDistributorFragment : Fragment() {
    ...
    private var adapter: TransaksiFirestoreRecyclerAdapter? = null
    ...
    private fun showDaftar() {
        binding.rvDistributor.layoutManager = LinearLayoutManager(
            activity,
        ...
        val options = FirestoreRecyclerOptions
            ...
            .build()
        adapter = TransaksiFirestoreRecyclerAdapter(options)
        binding.rvDistributor.adapter = adapter
    }
    ...
    private inner class ProductViewHolder(private val view: View) :
        ...
        fun setContentDistributor(
            ...
            val permintaan = view.findViewById<TextView>(R.id.permintaan)
            ...
            view.findViewById<RelativeLayout>(R.id.relShowall).setOnClickListener {
                if (CheckConection.isNetworkAvailable(activity!!)) {
                    val intent = Intent(activity, DetailDistributorActivity::class.java)
            ...
            Constants.REQUEST_PRODUCT_DB.whereEqualTo("distributorId", id)
                .whereEqualTo("selesaiDiterima", false).get().addOnSuccessListener {
                if (!it.isEmpty) {
                    permintaan.visibility = View.VISIBLE
                    permintaan.text = "${it.size()} Permintaan"
                } else {
                    permintaan.visibility = View.INVISIBLE
                }
            }
    ...
    private inner class TransaksiFirestoreRecyclerAdapter(options: FirestoreRecyclerOptions<Distributor>) :
